...
const Profile = ({ history, match }) => {
    ...
    const loadProfile = () => {
        axios({
            ...
            .then((profile) => {
                const { name, level, year, status } = profile.data;
                setValues({ name, level, year, status });
            })
            .catch((error) => {
        ...
        axios({
            method: "GET",
            url: `${process.env.REACT_APP_API}/match/list-by-team/${match.params.id}`,
            headers: { Authorization: `Bearer ${token}` },
        })
            .then((response) => {
            ...
            <div className="col-lg-12 col-md-12 col-sm-12">
                <table className="table">
                    <tbody>
                        ...
                        <tr>
                            <td>Team status</td>
                            <td>{values.status == 1 ? 'Playing' : 'Not playing'}</td>
                        </tr>
                    </tbody>
                </table>
