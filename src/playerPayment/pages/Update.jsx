...
const UpdatePlayerPayment = ({ match, history }) => {
    ...
    const loadProfile = () => {
        axios({
            ...
            .then((response) => {
                console.log("PLAYER PAYMENT LOADED", response);
                const {
                    amount,
                    reference,
                    type,
            ...
            .catch((error) => {
                // console.log("PLAYER PAYMENT UPDATE ERROR", error.response.data.error);
    ...
    const doSubmit = (event) => {
        ...
        axios({
            method: "PUT",
            url: `${process.env.REACT_APP_API}/player-payment/update/${match.params.id}`,
            headers: { Authorization: `Bearer ${token}` },
            data: {
                amount,
                reference,
            ...
            .then((response) => {
                // console.log("PLAYER PAYMENT UPDATED", response);
                toast.error(response.data.message);
                // console.log(profileID)
                history.push(`/admin/player/profile-admin-view/${profileID}`);
            })
            .catch((error) => {
                console.log("PLAYER PAYMENT UPDATE ERROR", error.response.data.error);
                setValues({ ...values, btnLabel: "Update" });
    ...
    const updatePaymentForm = () => (
        <form>
            ...
            <Link to={`/admin/player/profile-admin-view/${profileID}`} className="btn btn-success btn-md mr-3">Back to profile</Link>
            <Link to="#" className="btn btn-primary btn-md" onClick={doSubmit}>{btnLabel}</Link>
        </form>
    );
    return (
        <Layout>
            <div className="container-update-player-payment">
...
export default UpdatePlayerPayment;
